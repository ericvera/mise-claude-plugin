...
Ten steps, in order. Answer every **Skip when** line at the start of the run and again after each amendment; a yes → `mark .mise <step> skipped` before that step's turn comes.
...
3. The config's Mock conditions match → build `.mise/mock/` as a static HTML mock of every screen and state the work touches, its product name and look taken from the repo's own UI code and `CLAUDE.md`, and iterate on it with the owner. Ask about behavior and states; never about spacing, wording or layout.
4. Fold the answers into `goals.md` and record under `## Assumptions` every inference you made instead of asking.
5. Print at most 3 lines — the issue, the fix or approach you propose, what the run will skip — with the mock when there is one, and ask "Approve, or what should change?" Feedback, questions and silence are not approval. On approval: `mark .mise goals done`, commit.
...
5. More than 8 tasks → also write `.mise/requirements.md`: user-facing behavior as testable `REQ-<AREA>-<n>` lines grouped by area, cited in the task index.
6. `mark .mise spec done`, commit.

## critic

Skip when: the spec step was skipped **and** the work touches nothing hard to undo.

Spawn one `critic` per round over `spec.md` (plus `requirements.md` when present), and revise between rounds. Stop when a round returns no new blocking finding; hard cap 5 rounds. A round that returns a blocking finding is never the quiet round that stops the loop. Then `mark .mise critic done`, commit.
...
1. Spawn an `implementer` on the task file, with `.mise/progress.md` as its progress log. `blocked` → stop and relay it. `stuck` → one fresh implementer carrying the failure report; a second failure of either kind → stop.
...
4. `git mv` the task file into `.mise/tasks/done/` and commit `mise: task <id> done`.
...
Spawn `adherence` subagents per family listed there, in batches over the branch diff, each with that family's examples file, each appending only its flagged rows to that family's section of `.mise/adherence.md`. Flagged rows → one fix round through an implementer for that family, then re-run that family over the flagged files alone; at most 2 rounds per family, then record what is still flagged and move on. `mark .mise adherence done`, commit.

## sweep

Skip when: never.

Spawn one `sweep` subagent over the branch diff with `goals.md` and `spec.md`. Fix items → one implementer, then re-run the sweep once over the files it changed. `mark .mise sweep done`, commit.

## review

Skip when: never.

Write `.mise/review.md`: one line per task — what changed and how to verify it, the detail left in `progress.md` — then the open assumptions from `goals.md` and `spec.md` and the amendments so far, at most 60 lines in all, which is one sitting's reading; past that, one line per area instead of per task. Print at most 5 lines — what to look at, and where — and stop. The run waits here. Feedback arrives in chat, or through the `delta:review-notes` skill's contract wherever that skill is installed and the owner says there are notes.
...
State a factual objection once, briefly, then do what the owner decides. The stage ends when the owner says done, accept or ship: `mark .mise review done`, commit.

## gate

Skip when: never.

Run once, in order: the config's `Format`, `Check` and `Unit tests`, then the e2e suite when a `required` Skills & guides entry names one — through that entry, never directly. Never re-run a command that has had no edit since its last run.

A failure gets one repair through an implementer and one re-run of the gate. A second failure → print it and hand it to the owner as review feedback, handled by the rules above; the gate runs again once the fix lands. On success: `mark .mise gate done`, commit.

## close

Skip when: never.

`log {"event":"close","codeLines":…,"artifactLines":…}`, copy `.mise/ledger.jsonl` to `.claude/mise-ledger/<branch>.jsonl`, then delete `.mise/` and commit `mise: close`. Ship per the config's `Ship` value, `pr` where it has none: `pr` → push and open a pull request summarizing the work; `merge (<style>)` → merge into the default branch in that style; `off` → report the branch ready. Shipping fails → report the branch and the action you tried.
