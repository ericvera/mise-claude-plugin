...
`acceptance` triggers the workflow's **acceptance pass and cleanup**: the finished work is checked against its docs and, once the user confirms, the mise directory (artifacts, plan, state file — all of it) is deleted and the deletion committed. Artifacts are committed to the feature branch at every checkpoint so any checkout can resume the work, but they never reach the merged history — the code, tests, and git log are the durable output. An interrupted cleanup (crash between the user's confirmation and the deletion) needs no special state: the next run reports `acceptance` again and the user re-confirms.
