...
- For each line ask whether removing it would cause a mistake; if not, cut it. Every shipped line maps to evidence under `docs/line-evidence.md`'s rules, and the per-file budgets its checker enforces are ceilings, not targets.
...
- Changing any of these files — or a project's rule files — goes through `/mise:retro`'s change protocol.
