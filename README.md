...
Project details live in the generated `.claude/mise-config.md`. Required: the `## Quality commands` — Format, Check (lint, typecheck and the build where you have one) and Unit tests. Optional: `Ship` (`merge (squash | merge commit | rebase)` or `off`; no value means `pr`), `## Mock conditions`, `## Test exceptions`, `## Skills & guides`, `## Adherence`. Setup infers the quality commands from your package manifest and asks two questions: confirm them, and how to ship. The rest is fixed: work in flight lives in `.mise/` on its branch, ledgers archive to `.claude/mise-ledger/`, branches are `feat/<slug>` and `fix/<slug>`.
...
| **spec**           | the work fits one implementer's context | the driver reads the code itself, then writes `spec.md` (What, Design, Hard-to-undo, Task index) and one file per task; a `requirements.md` only past 8 tasks                                                                             |
| **critic**         | no spec and nothing hard to undo        | rounds over the spec; stops when a round finds nothing new that blocks; hard cap 5                                                                                                                                                        |
| **execute**        | never                                   | per task: `implementer` (a bug fix's regression test first, seen to fail) → `Unit tests` → `reviewer` (per task when the work spans 3+ modules, else over the whole diff in 20-file batches) → at most 2 fix rounds → commit              |
...
| **gate**           | never                                   | Format, Check, Unit tests, e2e through its required guide — once, one repair, one re-run                                                                                                                                                  |
| **close**          | never                                   | ledger copied to `.claude/mise-ledger/`; `.mise/` removed; ship per `Ship`                                                                                                                                                                |
